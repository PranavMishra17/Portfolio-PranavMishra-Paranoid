...
function Sample() {
  return <p className="bomb-sample">Sample titles.</p>;
}
  ...
  switch (k) {
    ...
    case 'laptop':
      return (
        <>
          {ABOUT_LONG.map((l) => (
            <p className="bomb-p" key={l}>
              {l}
            </p>
          ))}
          <p className="bomb-links">
            {LINKS.map((l) => (
              <a className="bomb-link" key={l.label} href={l.href} target={l.href.startsWith('/') || l.href.startsWith('mailto') ? undefined : '_blank'} rel="noopener noreferrer">
                {l.label}
              </a>
            ))}
          </p>
    ...
    case 'monitorA':
      return (
        <ul className="bomb-plist">
          <li>
            <h3 className="bomb-plist-t">
              Alfred_ <span className="bomb-plist-when">{ALFRED_ROLE.duration.replace('Present', 'now')}</span>
            </h3>
            <p className="bomb-plist-sub">
              {ALFRED_ROLE.title}, {ALFRED_ROLE.location}
            </p>
            <p className="bomb-plist-line">{ALFRED.lead}</p>
            <ul className="bomb-bullets">
              {ALFRED.more.map((d) => (
                <li key={d}>{d}</li>
              ))}
            </ul>
          ...
          {BEFORE.map((r) => (
            <li key={r.id}>
              <h3 className="bomb-plist-t">
                {r.company} <span className="bomb-plist-when">{r.when}</span>
              </h3>
              <p className="bomb-plist-sub">{r.title}</p>
              <p className="bomb-plist-line">{r.line}</p>
            </li>
          ))}
        </ul>
      );
    case 'monitorB':
      return (
        <ul className="bomb-plist">
          {PAPERS.map((p) => (
            <li key={p.id}>
              <h3 className="bomb-plist-t">{p.short}</h3>
              <p className="bomb-plist-sub">
                {p.status}, {p.venue}
                {p.citations ? `, ${p.citations} citations` : ''}
              </p>
              <p className="bomb-plist-line">{p.line}</p>
              <p className="bomb-links">
                {p.pdf ? (
                  <a className="bomb-link" href={p.pdf} target="_blank" rel="noopener noreferrer">
                    Paper
                  </a>
                ) : null}
                {p.code ? (
                  <a className="bomb-link" href={p.code} target="_blank" rel="noopener noreferrer">
                    Code
                  </a>
                ) : null}
              </p>
    ...
    case 'poster3': {
      ...
      return (
        <>
          <h3 className="bomb-plist-t">{f.title}</h3>
          <p className="bomb-p">{f.why}</p>
          <Sample />
        </>
      );
    }
    case 'trophies':
      return (
        <ul className="bomb-plist">
          {TROPHIES.map((t) => (
            <li key={t.id}>
              <div className="bomb-print">
                <img src={t.image} alt="" loading="lazy" />
              </div>
              <h3 className="bomb-plist-t">{t.title}</h3>
              <p className="bomb-plist-line">{t.line}</p>
            </li>
          ))}
        </ul>
      );
      ...
      return (
        <>
          <ul className="bomb-plist">
            {BOOKS.map((b) => (
              <li key={b.title}>
                <h3 className="bomb-plist-t">{b.title}</h3>
                <p className="bomb-plist-sub">
                  {b.author}. {b.state}.
                </p>
              </li>
      ...
      return (
        <>
          <ul className="bomb-plist">
            {GAMES.map((g) => (
              <li key={g.title}>
                <h3 className="bomb-plist-t">{g.title}</h3>
                <p className="bomb-plist-sub">{g.note}</p>
              </li>
            ))}
          </ul>
          <Sample />
      ...
      return (
        <>
          <ul className="bomb-plist">
            {MAGNETS.map((mg) => (
              <li key={mg.place}>
                <h3 className="bomb-plist-t">{mg.place}</h3>
                <p className="bomb-plist-sub">{mg.memory}</p>
              </li>
            ))}
          </ul>
          <Sample />
    ...
    case 'photo':
      return (
        <>
          <p className="bomb-p">{FAMILY_PHOTO.caption}</p>
          <Sample />
        </>
      );
    case 'ball':
      return (
        <>
          <p className="bomb-p">{FOOTBALL.line}</p>
          <p className="bomb-p bomb-soft">{FOOTBALL.extra}</p>
        </>
      );
    default:
      return null;
...
export default function Room({ active }) {
  ...
  useEffect(() => {
    ...
    const fit = () => {
      ...
      // fill the width; if that is shorter than the screen, fill the height instead
      let cw = vw;
      let ch = Math.round((vw * H) / W);
      if (ch < vh) {
        ch = vh;
        cw = Math.round((vh * W) / H);
      }
      canvas.width = cw;
      canvas.height = ch;
  ...
  return (
    <div className={`bomb-room${active ? ' live' : ''}`}>
      <canvas ref={canvasRef} className={`bomb-pixels${hover ? ' hot' : ''}`} onMouseMove={onMove} onMouseLeave={() => setHover(null)} onClick={onClick} aria-label="A pixel-art drawing of my room" role="img" />
      <p className="bomb-caption" aria-live="polite">
        {hover ? hover.label : ''}
      </p>
